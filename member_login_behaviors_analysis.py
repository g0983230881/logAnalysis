...
member_amount = growing_datas[0]['total']
...
APIcount = db_edumarket.apilog.find({'since' : {'$gte': FirstDate_109Year, '$lt': FirstDate_NextMonth}}).count() + db_edumarket.resources_usage_tpcooc.find({'created_at': {'$gt': FirstDate_109Year, '$lt': FirstDate_NextMonth}}).count()


# 計算自109年起累計至本月之點閱數
resources_click_begin109Year = db_marketanalysis.search_log2.find({'class_code':'R', "since" : {'$gte': FirstDate_109Year, '$lt':FirstDate_NextMonth}}).count()
# 計算自109年起累計至本月之下載數
resources_download_begin109Year = db_marketanalysis.search_log2.find({'class_code':{'$in' : ['F','L']}, "since" : {'$gte' : FirstDate_109Year, '$lt':FirstDate_NextMonth}}).count()
# 計算自109年起累計至本月之點讚數
resources_good_begin109Year = db_marketanalysis.search_log2.find({'class_code':'MC', "since" : {'$gte' : FirstDate_109Year, '$lt':FirstDate_NextMonth}}).count()
# 計算自109年起累計至本月之分享數
resources_share_begin109Year = db_marketanalysis.search_log2.find({'class_code':{'$in' : ['SE','SG','SF','ST','SL']}, "since" : {'$gte' : FirstDate_109Year, '$lt':FirstDate_NextMonth}}).count()
