...
for user_id in userIdlist:
	...
	if (is_exists >= 1):  ## 有last_log, 從上次執行最後一筆的時間往後繼續跑
		...
		identifyMemberSession = db_marketanalysis.identify_member.find({'user_id' : user_id})
		df_identifyMemberSession = pd.DataFrame(identifyMemberSession)
		Series_identifyMemberSession = df_identifyMemberSession['session_id']
		for identifymember in Series_identifyMemberSession:
			#print(str(identifymember['user_id']) + '   ' + identifymember['session_id'])
			session_id.append(identifymember)
