...
# print('deltaTime: ', endTime - startTime)
# deltaTime單位: days,seconds,microseconds
...
cursor1.execute(resources_createdate_groupSQL)
totalrecord = cursor1.fetchall()
...
for record in totalrecord :
	...
	doc = {str(k):v for k,v in record.items()}
	createDate = doc['createdate']
	#createDate 2004-07-30
	...
	cursor2.execute(resources_selectByCreatedateSQL.format(createDate))
	resources = cursor2.fetchall()
	# print(resources[:3])
	# [{u'city_id': 51, u'school_id': 0, u'id': 2, u'member_id': 13712}, 
  	#  {u'city_id': 51, u'school_id': 0, u'id': 15, u'member_id': 13712}, 
  	#  {u'city_id': 51, u'school_id': 0, u'id': 21, u'member_id': 13712}]
	...
	for resource in resources :
		resources_id = str(resource.get('id'))
		#統計每天各縣市上傳的資源筆數
		# IfStart = datetime.datetime.now()
		city_id = str(resource.get('city_id'))
		...
		#統計每天各縣市中各個學校上傳的資源筆數
		school_id = str(resource.get('school_id'))
		if (school_id != "0") : 
		...
		#統計每天每個人上傳資源的筆數
		member_id = str(resource.get('member_id'))
		if (int(member_id) > 0) :
		...
		DomainCursor.execute(resources_selectDomainSQL.format(resources_id))
		Disciplines = DomainCursor.fetchall()
		...
		#統計每天各縣市上傳資源的適用年級筆數
		GradeCursor.execute(resources_selectGradeSQL.format(resources_id))
		Grades = GradeCursor.fetchall()
