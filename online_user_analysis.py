...
for server_name in server_li:
	...
	if (is_lastlog_exists > 0):
		...
		while end_dt > start_dt:
			...
			emin_mongo_count = 0
			emin_mysql_count = 0
			data_count = col_onlineUser.find({"$and": [{"server": server_name}, {"time":{'$gte':this_start_dt, '$lte':this_end_dt}}]}).count()
...
for server_name in server_li:
	...
	if (is_lastlog_exists > 0):
		...
		while end_dt > start_dt:
			...
			ehour_mongo_count = 0
			ehour_mysql_count = 0
			data_count = col_onMinute.find({"$and": [{"server": server_name}, {"time":{'$gte':this_start_dt, '$lte':this_end_dt}}]}).count()
