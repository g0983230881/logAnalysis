#!/usr/bin/python3.8
# -*- coding: utf-8 -*-
#程式名稱：sync_resource_link2.py
#功能說明：檢查教學資源(isp資源編號大於1696009)的連結或附件是否相同
#輸入參數：無
...
#ResourceSQL = "select id,title,`desc`,bmtype,workflow_id,member_id from resources where isdelete='N' and workflow_id in ('WK2005','WK2008') and id>=1709589 and id<=1710745"
...
renameAttach = open("../output/sync_resource_attach_rename3.txt","w+")
resfile1 = open("../output/sync_resource_link3.txt","w+")
#cursor1.execute(ResourceSQL)
#resourcesRows = cursor1.fetchall()
#for resources in resourcesRows :
resources_mapping = open("../conf/resources_mapping.txt", "r")
for line in resources_mapping.readlines() :
	line = line.strip()
	resourcesIdMapping = line.split(',')
	resources_id = resourcesIdMapping[1]

	ispcursor1.execute(ISPResourceSQL, (resourcesIdMapping[0]))
	ispResourceRow = ispcursor1.fetchone()
	if ispResourceRow is not None:
		cursor2.execute(ResourceObjSQL.format(resourcesIdMapping[1]))
