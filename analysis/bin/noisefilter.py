...
if __name__ == "__main__" :
	...
	print('return IP, Pattern finished.')
	...
	try:
		...
		with client.start_session() as session:
			...
			for _searchlog in searchlog_data:
				...
				if len(_searchlog['session_id']) > 0:
					...
					identify_member.update_one(
						...
						upsert=True,
						session = session)
					# print('identify_member1 update one data.')
				...
				#將沒有被過濾的日誌記錄儲存
				search_log.insert_one(_searchlog, session = session)
				db_marketanalysis.lastlog.update_one({'type':'searchlog_lastId'},{"$set":{'id':_searchlog['_id'],'type':'searchlog_lastId'}},upsert=True,session = session)
				
				count += 1
				if(count == 10000):
					print('db_marketanalysis update 10000 datas.')
					count = 0
				'''
				#寫入使用者追踪及識別資料
				if len(_searchlog['session_id']) > 0:
				...
				if count > 10 :
					break"""
			print('searchlog_data for loop finished.')
			...
			identify_member_datas = identify_member.find({'user_id' : {'$gt' : 0}}, session=session)
			for identify_member_data in identify_member_datas:
				...
				identify_member.update_one(
					...
					upsert=False,
					session = session)
				print('identify_member2 update one data')
			print('identify_member_datas loop finished.')
