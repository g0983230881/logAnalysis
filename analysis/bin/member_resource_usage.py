...
print('userIdList distinct finished.')
...
for user_id in userIdlist :
	...
	print('session_id_count: ',len(session_id))
	...
	# 一次搜尋350000個id
	batch_size = 350000
	for i in range(0, len(session_id), batch_size):
		# 共分成18次, 完成搜索
		session_batch = session_id[i:i+batch_size]
		userBehaviorLog = db_marketanalysis.search_log2.find(
			{'session_id' : session_batch,
			"since": { "$gte": end_date, "$lt": begin_date}}
			).sort([
				('from_ip',1),
				('class_code',1), 
				('main_col',1), 
		...
		#userBehaviorLog讀取出來的document約282Mb, 但官方document最大大小設限是16Mb
...
client.close()
print('member_resource_usage finished.')